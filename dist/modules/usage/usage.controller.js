...
exports.incrementUsage = exports.getUsage = void 0;
...
const getUsage = async (req, res) => {
    try {
        ...
        const logs = await database_1.prisma.usageLog.findMany({
            where: {
                tenant_id: tenantId,
                created_at: { gte: firstDay },
            },
        });
        const totalCalls = logs.reduce((sum, log) => sum + log.api_calls, 0);
        const lastLog = logs.length > 0 ? logs[logs.length - 1] : null;
        const currentStorage = lastLog ? lastLog.storage_gb : 0;
        ...
        res.json({
            data: {
                ...
                api_calls_limit: license ? license.limits.api_calls_per_month : 0,
                users_count: await database_1.prisma.user.count({ where: { tenant_id: tenantId } }),
                users_limit: license ? license.limits.max_users : 0,
                storage_used_gb: currentStorage,
                storage_limit_gb: license ? license.limits.storage_gb : 0,
...
const incrementUsage = async (req, res) => {
    try {
        ...
        if (!tenantId)
            return res.status(400).json({ error: "User has no associate tenant" });
        const log = await database_1.prisma.usageLog.create({
            data: {
                tenant_id: tenantId,
                api_calls: 1,
            },
